...
if [ "$?" -eq "1" ]; then
$EMACSCLIENT --create-frame --no-wait --alternate-editor="" "$@" ; open -a emacs
else
$EMACSCLIENT --no-wait --alternate-editor="" "$@" > /dev/null 2>&1; open -a emacs
fi
#+END_SRC
...
(defun org-mode-export-journal ()
  "Export links document to HTML automatically when 'journal.org' is changed"
  (when (equal (buffer-file-name) "/Users/rakhim/Google Drive/Knowledgebase/org/journal.org")
    (progn
      (org-html-export-to-html)
      (message "HTML exported"))))

(add-hook 'after-save-hook 'org-mode-export-links)
(add-hook 'after-save-hook 'org-mode-export-journal)
#+END_SRC
